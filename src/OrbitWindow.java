	       ...
	       WindowListener, ClipboardOwner
{
	...
	menuAbout = new MenuItem("About Orbit Chat");
	...
	myConstraints.gridheight = 1; myConstraints.gridwidth = 2;
	myConstraints.anchor = GridBagConstraints.SOUTHWEST;
	myConstraints.fill = GridBagConstraints.HORIZONTAL;
	myConstraints.weightx = 0.0; myConstraints.weighty = 0.0;
	myConstraints.insets.right = 5; myConstraints.insets.left = 0;
	myConstraints.insets.top = 0; myConstraints.insets.bottom = 5;
	...
	try {
	    URL iconUrl =
		new URL(OrbitURL.getProtocol(), OrbitURL.getHost(),
			OrbitURL.getFile() + "OrbitIcon.jpg");
	...
	if (adminConsole)
	    roomOwner(true);
	setTitle("Java Chat v" + Orbit.VERSION + " - online at " + host);
	return;
    }
	...
	roomOwner(false);
	setTitle("Java Chat v" + Orbit.VERSION + " - offline");
	return;
    }
	...
	if (E.getSource() == menuAbout)
	    {
String abouttext = new String("Orbit v" + Orbit.VERSION + "\nCopyright ORBIT IT CONSULTING (P) LTD.\n\n");
		
		new OrbitTextDialog(this, "About Orbit Chat", 
				      abouttext, 60, 22,
	...
	if (E.getSource() == colorChoice)
	    {
		canvas.drawColor =
		    OrbitCanvas.colourArray[colorChoice.getSelectedIndex()];
		return;
	    }            
	...
	if (E.getSource() == freehand)
	    {
		canvas.drawType = OrbitCanvas.FREEHAND;
		thickness.setEnabled(true);
		fillType.setEnabled(false);
		return;
	    }
	...
	if (E.getSource() == line)
	    {
		canvas.drawType = OrbitCanvas.LINE;
		thickness.setEnabled(true);
		fillType.setEnabled(false);
		return;
	    }
	...
	if (E.getSource() == oval)
	    {
		canvas.drawType = OrbitCanvas.OVAL;
		thickness.setEnabled(true);
	...
	if (E.getSource() == rectangle)
	    {
		canvas.drawType = OrbitCanvas.RECTANGLE;
		thickness.setEnabled(true);
		fillType.setEnabled(true);
		return;
	    }
	...
	if (E.getSource() == text)
	    {
		canvas.drawType = OrbitCanvas.TEXT;
